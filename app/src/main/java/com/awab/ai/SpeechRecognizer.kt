...
class SpeechRecognizer(private val context: Context) {
    ...
    private fun recordAndRecognize() {
        ...
        val inputShape = interpreter?.getInputTensor(0)?.shape() ?: intArrayOf(1, 128000)
        val requiredSize = inputShape[1] // 128000 samples = 8 seconds
        val minSize = requiredSize / 4 // 2 seconds minimum (can be adjusted)
        
        Log.d(TAG, "📊 بدء التسجيل - الحد الأدنى: ${minSize/16000}s، الحد الأقصى: ${requiredSize/16000}s")
        ...
        try {
            while (isRecording) {
                ...
                if (readSize > 0) {
                    ...
                    Log.d(TAG, "📊 تم تسجيل: $currentSize/$requiredSize عينة (${currentSize/16000}s)")
                    ...
                    // 1. Reached full 8 seconds
                    // 2. Have minimum 2s AND detected silence for 0.6s
                    // 3. User stopped recording
                    
                    val hasEnoughAudio = currentSize >= minSize
                    val detectedSilence = silenceCount >= silenceDuration
                    val reachedMax = currentSize >= requiredSize
                    
                    if (reachedMax || (hasEnoughAudio && detectedSilence)) {
                        if (reachedMax) {
                            Log.d(TAG, "🎯 وصل إلى ${requiredSize/16000}s - بدء المعالجة...")
                        } else {
                            Log.d(TAG, "🎯 تم كشف سكوت بعد ${currentSize/16000}s - بدء المعالجة...")
                        }
            ...
            if (audioData.size >= minSize) {
                Log.d(TAG, "🎯 توقف التسجيل مع ${audioData.size/16000}s - معالجة نهائية...")
    ...
    private fun recognizeSpeech(audioData: ShortArray): String {
        try {
            ...
            // v19_ULTIMATE processing
            val requiredSize = inputShape[1] // 128000 samples = 8 seconds
            Log.d(TAG, "📊 Required: $requiredSize samples (${requiredSize/16000}s)")
