...
class CommandHandler(private val context: Context) {
    ...
    fun handleCommand(rawMessage: String): String {
        ...
        return when {
            ...
            lowerMessage.contains("debug") || lowerMessage.contains("ديباق") -> {
                val service = MyAccessibilityService.getInstance()
                if (service == null) "⚠️ خدمة إمكانية الوصول غير مفعّلة"
                else service.debugScreenTree()
            }
    ...
    private fun showCommandsList(): String {
        return """
📋 *قائمة الأوامر*
...
• شغل وضع الطيران
• شغل نقطة اتصال
• على الصوت / خفض الصوت / كتم الصوت
• سكرين شوت
• قلب الشاشة

👁️ *المراقبة*
• راقب [تطبيق] [N] مرات → انتظر ظهور [نص] ثم [أمر]
• أوقف مراقبة [تطبيق]
• المراقبات

⭐ = يحتاج تفعيل خدمة إمكانية الوصول
        """.trimIndent()
    }
